...
static void LoadExtByName(const char *extensionName) {
	...
	if(entry)
	{
		if(entry->LoadExtension)
		{
			...
			if(numFailed == 0)
			{
				*(entry->extensionVariable) = GLC_LOAD_SUCCEEDED;
			}
			else
			{
				*(entry->extensionVariable) = GLC_LOAD_SUCCEEDED + numFailed;
			}
		}
		else
		{
...
int glcLoadFunctions() {
	int numFailed = 0;
	ClearExtensionVars();
	
	_ptrc_glGetIntegerv = (void (CODEGEN_FUNCPTR *)(GLenum, GLint *))IntGetProcAddress("glGetIntegerv");
	if(!_ptrc_glGetIntegerv) return GLC_LOAD_FAILED;
	_ptrc_glGetStringi = (const GLubyte * (CODEGEN_FUNCPTR *)(GLenum, GLuint))IntGetProcAddress("glGetStringi");
	if(!_ptrc_glGetStringi) return GLC_LOAD_FAILED;
	
	ProcExtsFromExtList();
	numFailed = Load_Version_4_5();
	
	if(numFailed == 0)
		return GLC_LOAD_SUCCEEDED;
	else
		return GLC_LOAD_SUCCEEDED + numFailed;
}
...
int glcIsVersionGEQ(int majorVersion, int minorVersion)
{
	if(g_major_version == 0)
		GetGLVersion();
