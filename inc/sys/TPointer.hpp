...
namespace sys {
  ...
  template<typename T> class TPointer<T, EPointer::SHARED> {
    ...
    public:
      ...
      /* take ownership */
      template <typename Y, typename = typename std::enable_if<std::is_convertible<Y*,T*>::value>::type> TPointer(Y* pPointer) : mPointer{pPointer}, mCount{new uint32_t{0}} { if (mPointer) { ++(*mCount); } }
...
// @todo: DO NOT ALLOW weak pointer (w/ no info) here
    ...
    public: // cast operator
      ...
      explicit operator const  T() const { return *mPointer; }
    public:
