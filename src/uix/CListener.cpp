...
namespace uix {
  ...
  bool CListener::handle(CEvent* pEvent) {
    SYS_LOG_NFO("app::CListener::handle(CEvent*)::" << this << " EVENT:" << (int)(pEvent->type()));
  ...
  bool CListener::listens(const CEvent::EType& eEvent) const { 
    SYS_LOG_NFO("uix::CListener::listens(CEvent::EType&)::" << this << " EVT:" << int(eEvent) << " " << mHandlers.count(eEvent));
